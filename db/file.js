...
const FileDb = (() => {
  const getFileById = async (id, user) => {
    if (!id || !user)
      throw new Error("Please provide a file id and valid user");
  ...
  const uploadFile = async (file, folder, user) => {
    if (!file || !folder || !user)
      throw new Error(
        "Please provide a file, folder and valid user to upload file",
      );
  ...
  const updateFilename = async (file, name, user) => {
    if (!file || !name || !user)
      throw new Error(
        "Please provide a file, name and valid user to update file name",
      );
    return await prisma.file.update({
      where: { id: file.id, userId: user.id },
      data: { name },
    });
  };
  ...
  const deleteFile = async (file, user) => {
    if (!file || !user)
      throw new Error("Please provide a file and valid user to delete file");

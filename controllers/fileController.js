...
const fileController = (() => {
  const validateEditFile = [
    body("updatedFilename")
      ...
      .withMessage("New file name must be between 1 and 50 characters."),
  ];
  ...
  const validateMulterUpload = async (req, res, next) => {
    ...
    if (req.file.size > VALID_FILE_SIZE) {
      return res.render("pages/fileUpload", {
        folder,
        errors: [{ msg: "Please select a file that is smaller than 10MB" }],
      });
    }
    if (req.file.originalname.length > VALID_FILE_NAME_LENGTH) {
      return res.render("pages/fileUpload", {
        ...
        errors: [
          {
            msg: `Filename must be shorter than ${VALID_FILE_NAME_LENGTH} characters. Please rename and try again.`,
          },
        ],
      });
    }
  ...
  const handleUploadErrors = async (err, req, res, next) => {
    ...
    if (
      err.code === "P2002" ||
      (err.__isStorageError && err.message.includes("already exists"))
    ) {
    ...
    return res
      .status(400)
      .render("pages/fileUpload", { folder, errors: [{ msg: errorMessage }] });
  };
  ...
  const uploadFilePost = [
    ...
    async (req, res) => {
      ...
      const parentFolderId =
        folder.parent && folder.parent.id ? folder.parent.id : "";
  ...
  const editFilePost = [
    ...
    async (req, res) => {
      ...
      if (!errors.isEmpty())
        return res
          .status(400)
          .render("pages/fileEdit", { folder, file, errors: errors.array() });
  ...
  const handleDeleteErrors = async (err, req, res, next) => {
    ...
    return res.render("pages/fileDelete", {
      file,
      folder,
      errors: [{ msg: errorMessage }],
    });
  ...
  const deleteFilePost = [
    ...
    async (req, res) => {
      ...
      if (String(req.body.deleteMsg) !== "delete file")
        throw new Error("Incorrect delete message. Try again.");
  ...
  const downloadFileGet = async (req, res) => {
    ...
    const { data } = await supabase.storage
      .from("files")
      .createSignedUrl(file.url, 60, { download: file.name });
